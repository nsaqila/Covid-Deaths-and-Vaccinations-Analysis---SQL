...
from deaths
where continent is not null --make sure continent is not null
order by 2;
...
where 
	...
	and continent='Asia'	--want to see deaths in Asia
...
select
	...
	max(total_deaths) as total_death_count
from deaths
where continent is not null 	--to exclude continents in 'location'
...
-- overall total cases, total deaths, and death percentage globally
select 
...
-- join deaths and vaccinations table into one table
select *
from deaths dea
...
select distinct 	-- to make sure no duplicate values are repeated
	dea.continent,
	dea.location,
	dea.date,
	dea.population,
	coalesce(sum(distinct vac.new_vaccinations), 0) as new_vaccinations 	--distinct to make sure no duplicate vaccination records are repeated
from deaths dea
left join vaccinations vac
	on dea.location = vac.location
	and dea.date = vac.date
...
select distinct
	...
	dea.date,
	dea.population,
	coalesce(sum(distinct vac.new_vaccinations), 0) as new_vaccinations,
	sum(coalesce(sum(distinct vac.new_vaccinations), 0)) over (partition by dea.location 
